/* eslint-disable no-console -- one-off seed generator */
...
const insertHeader = `INSERT INTO job_seeker_profiles
...
const valueLines = rows.map((r, i) => {
  const [ini, name, role, shortD, longD, skills, price, rating, rev, acc, bLabel, bTone, sort] = r;
  const starLabel = i % 7 === 3 ? stars4 : stars5;
  const bl = bLabel == null ? "NULL" : `'${sqlString(bLabel)}'`;
  const bt = bTone == null ? "NULL" : `'${sqlString(bTone)}'`;
  return `  (
    '${sqlString(ini)}',
    '${sqlString(name)}',
    '${sqlString(role)}',
...
const insertBlock = insertHeader + valueLines.join(",\n") + ";\n";

const workbenchPreamble = `-- ① database/schema.sql дараа нь энэ файлыг ажиллуул.
...
const workbench = `${workbenchPreamble}${insertBlock}\n${demoUserAndEmployees}`;
const addJobPreamble = `-- job_seeker_profiles-ийг дахин бүтээж 30 мөр оруулна. USE: Workbench
USE zeel_platform;

${createTableDdl}

`;
const addJob = `${addJobPreamble}\n${insertBlock}`;

fs.writeFileSync(path.join(root, "database", "workbench-seed.sql"), workbench, "utf8");
fs.writeFileSync(path.join(root, "database", "add-job-seeker-profiles.sql"), addJob, "utf8");
console.log("Wrote database/workbench-seed.sql and database/add-job-seeker-profiles.sql");